...
import { PanelBody, SearchControl, Button, Spinner } from '@wordpress/components';
...
export default function Edit({ attributes, setAttributes }) {
	...
	return (
		<div {...useBlockProps()}>
			<InspectorControls>
				<PanelBody title={ __('Read More: Select Post', 'dmg') } initialOpen>
					<SearchControl
						...
						placeholder={ __('Search by title or paste an ID', 'dmg') }
					/>
					...
					{ hasResolved && posts.length > 0 && (
						<div>
							{ showingRecent && (
								<h4 className="dmg-subheading">{ __('Recent Articles', 'dmg') }</h4>
							)}

							<div role="list" className="dmg-post-list">
								{ posts.map((post, idx) => (
									<div role="listitem" key={post.id} className="dmg-post-item">
										<Button
											variant="link"
										...
										{ idx < posts.length - 1 && <hr className="dmg-post-sep" /> }
									</div>
								))}
							</div>
...
function SelectedPreview({ postId }) {
	const { records: postArr, isResolving } = useEntityRecords('postType', 'post', {
		include: [postId],
		status: 'publish',
		_fields: ['id', 'title', 'link', 'status'],
		per_page: 1,
	...
	if (isResolving) return <p>{ __('Loading…', 'dmg') }</p>;

	const post = Array.isArray(postArr) ? postArr[0] : null;
	if (!post) return <p>{ __('Post not found or not published.', 'dmg') }</p>;

	const title = (post.title?.rendered ?? '').replace(/<[^>]*>/g, '');
	return (
		<p className="dmg-read-more">
			{ __('Read More: ', 'dmg') }
			<a href={post.link} target="_blank" rel="noreferrer noopener">{title}</a>
		</p>
	);
}
